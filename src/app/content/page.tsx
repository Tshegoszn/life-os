...
import { Camera, Plus, X, Lightbulb, FileText, TrendingUp, Loader } from 'lucide-react'
import { Card, Button, Badge, SectionHeader, PageWrapper, Input, Select } from '@/components/ui'
...
const STATUS_COLOR: Record<string, 'gray' | 'yellow' | 'lav' | 'green'> = {
  idea: 'gray', draft: 'yellow', ready: 'lav', posted: 'green',
}
const PLATFORM_BG: Record<string, string> = {
  TikTok: '#FBEAF0', Instagram: '#EEE8F5', Both: '#EAF3DE',
}
const PLATFORM_COLOR: Record<string, string> = {
  TikTok: '#EA4C89', Instagram: '#7B5EA7', Both: '#27500A',
}
...
export default function ContentPage() {
  ...
  const [captions, setCaptions] = useState([
    { id: 1, title: 'Motivational Monday', text: 'She built her empire one decision at a time. Be her. 💫 #mondaymotivation #girlboss' },
    { id: 2, title: 'Income reveal hook', text: 'I made R24k last month from my laptop. Here is exactly how 👇 #incomereveal #freelance' },
    { id: 3, title: 'Soft life aesthetic', text: 'Building a life so beautiful I never need a vacation from it 🌸 #softlife #lifestyle' },
  ])
  const [activeTab, setActiveTab] = useState('calendar')
  const [showAdd, setShowAdd] = useState(false)
  const [showCaption, setShowCaption] = useState(false)
  const [newPost, setNewPost] = useState<Omit<Post, 'id'>>({
    platform: 'Instagram', type: 'Reel', caption: '', date: '', status: 'idea', tags: [],
  })
  ...
  const updateStatus = async (id: string) => {
    ...
    if (!post) return
    const status = order[(order.indexOf(post.status) + 1) % order.length]
  ...
  return (
    <PageWrapper title="Content Planner" subtitle="Create content that converts and inspires 📱">
      ...
      <div className="flex items-center justify-between mb-4 border-b" style={{ borderColor: '#F5C8E2' }}>
        ...
        <div className="pb-1">
          <Button onClick={() => setShowAdd((v) => !v)}><Plus size={13} /> New post</Button>
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-4">
            <Card className="ring-1 ring-[#EA4C89]">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <Select label="Platform" value={newPost.platform}
                  onChange={(v) => setNewPost((x) => ({ ...x, platform: v as Post['platform'] }))}
                  options={PLATFORMS.map((p) => ({ value: p, label: p }))} />
                <Select label="Type" value={newPost.type}
                  onChange={(v) => setNewPost((x) => ({ ...x, type: v }))}
                  options={TYPES.map((t) => ({ value: t, label: t }))} />
                <Select label="Status" value={newPost.status}
                ...
                <Input label="Date" value={newPost.date}
                  onChange={(v) => setNewPost((x) => ({ ...x, date: v }))} type="date" />
                <div className="col-span-2">
                  <label className="text-[11px] font-medium mb-1 block" style={{ color: '#C73E74' }}>Caption</label>
                  <textarea value={newPost.caption}
                    onChange={(e) => setNewPost((x) => ({ ...x, caption: e.target.value }))}
                    placeholder="Write your caption here..." rows={3}
                ...
                <div className="col-span-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {newPost.tags.map((t) => (
                      <span key={t} className="text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1"
        ...
        <div className="space-y-3">
          {posts.length === 0 && (
            <div className="text-center py-16 text-xs" style={{ color: '#D8C1E8' }}>
              No content yet — add your first post 📱
            </div>
          )}
          {posts.map((p, i) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className="text-[10px]" style={{ color: '#888' }}>📅 {p.date || 'No date'}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {p.tags?.map((t) => (
                        <span key={t} className="text-[10px]" style={{ color: '#D8C1E8' }}>#{t}</span>
                      ))}
                    </div>
                  </div>
              ...
              <motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.04 }}>
                <Card hover className="group">
                  <div className="flex items-start gap-2">
                    ...
                    <button onClick={() => setIdeas((is) => is.filter((_, j) => j !== i))}
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-4">
                <Card>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={() => setShowCaption(false)}><X size={14} style={{ color: '#C73E74' }} /></button>
                  </div>
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <textarea value={newCapText} onChange={(e) => setNewCapText(e.target.value)} rows={4}
                        className="w-full border rounded-2xl px-3 py-2 text-xs resize-none focus:outline-none"
                        style={{ borderColor: '#F5C8E2', background: '#FFF7FB', fontFamily: 'Outfit,sans-serif' }} />
                    </div>
              ...
              <motion.div key={c.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Card hover className="group">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button onClick={() => setCaptions((cs) => cs.filter((x) => x.id !== c.id))}
                      className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <X size={12} style={{ color: '#C73E74' }} />
                    </button>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {(['idea', 'draft', 'ready', 'posted'] as const).map((s) => {
              ...
              return (
                <div key={s} className="flex justify-between items-center py-2 border-b last:border-0"
                  ...
                  <span className="text-xs font-medium" style={{ color: '#1A1A1A' }}>{count} posts</span>
                </div>
              )
            })}
          </Card>
          <Card className="lg:col-span-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                'Write caption', 'Add hashtags', 'Design thumbnail',
                'Record or edit video', 'Schedule post', 'Add location',
                'Tag collaborators', 'Add to calendar',
              ].map((item) => (
  ...
  return (
    <div onClick={() => setChecked((v) => !v)}
      ...
      <div className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-all"
        style={{ background: checked ? '#EA4C89' : 'transparent', borderColor: checked ? '#EA4C89' : '#F5C8E2' }}>
        {checked && <span className="text-white text-[9px]">✓</span>}
      </div>
      <span className="text-[11px]" style={{ color: checked ? '#EA4C89' : '#555' }}>{label}</span>
    </div>
  )
}