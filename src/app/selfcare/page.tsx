...
import { Heart, Droplets, Moon, Sun, Plus, Sparkles } from 'lucide-react'
import {
  Card,
  Button,
  ProgressBar,
...
type SleepEntry = { id: number; date: string; hours: number; quality: number }

export default function SelfCarePage() {
  const [skincare, setSkincare] = useState<SkinStep[]>([
    { id: 1, text: 'Gentle cleanser', done: true, am: true },
    { id: 2, text: 'Vitamin C serum', done: true, am: true },
  ...
  const [wellness, setWellness] = useState<WellnessItem[]>([
    { id: 1, text: 'Morning stretch or yoga', done: true },
    { id: 2, text: 'Drink 2L water', done: true },
  ...
  const [sleep, setSleep] = useState<SleepEntry[]>([
    { id: 1, date: '2026-05-25', hours: 7.5, quality: 4 },
    { id: 2, date: '2026-05-24', hours: 6, quality: 2 },
    { id: 3, date: '2026-05-23', hours: 8, quality: 5 },
    { id: 4, date: '2026-05-22', hours: 7, quality: 3 },
    { id: 5, date: '2026-05-21', hours: 9, quality: 5 },
  ])
  ...
  const avgSleep = sleep.reduce((a, b) => a + b.hours, 0) / sleep.length
  const avgQuality = sleep.reduce((a, b) => a + b.quality, 0) / sleep.length
    ...
    setSleep((s) => [
      {
        id: Date.now(),
        date: newSleep.date,
        hours: Number(newSleep.hours),
        quality: Number(newSleep.quality),
      },
      ...s,
    ])
        ...
        {[
          {
            ...
            value: `${Math.round((wellnessDone / wellness.length) * 100)}%`,
            sub: `${wellnessDone}/${wellness.length} done today`,
          },
          {
            label: 'Skincare today',
            value: `${amDone + pmDone}/${amTotal + pmTotal}`,
            sub: 'AM + PM steps',
          },
          {
            label: 'Avg sleep',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5 mb-3">
            {skincare.filter((s) => s.am).map((s) => (
              <div
                ...
                onClick={() =>
                  setSkincare((sk) =>
                    sk.map((x) => (x.id === s.id ? { ...x, done: !x.done } : x))
                  )
                }
              ...
              >
                <div
                  ...
                  style={{
                    background: s.done ? '#EA4C89' : 'transparent',
                    borderColor: s.done ? '#EA4C89' : '#F5C8E2',
                    color: '#fff',
                  }}
                >
                  {s.done && '✓'}
                ...
                <span
                  className={`text-xs ${s.done ? 'line-through text-gray-300' : ''}`}
                  style={{ color: s.done ? undefined : '#555' }}
                >
                  {s.text}
                </span>
              </div>
            ...
            {skincare.filter((s) => !s.am).map((s) => (
              ...
              >
                <div
                  className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 border transition-all text-[10px]"
                  style={{
                    background: s.done ? '#D8C1E8' : 'transparent',
                    borderColor: s.done ? '#D8C1E8' : '#F5C8E2',
                    color: '#fff',
                  }}
                >
                  {s.done && '✓'}
        ...
        <Card>
          ...
          <div className="grid grid-cols-4 gap-2 my-3">
            {Array.from({ length: 8 }, (_, i) => (
              <button
                ...
                style={{
                  background: i < water ? '#EEE8F5' : '#FFF7FB',
                  border: `0.5px solid ${i < water ? '#D8C1E8' : '#F5C8E2'}`,
                }}
              >
                <Droplets
                  size={16}
                  style={{ color: i < water ? '#D8C1E8' : '#F5C8E2' }}
                />
                <span className="text-[9px]" style={{ color: i < water ? '#7B5EA7' : '#ccc' }}>
                  {i + 1}
                </span>
              </button>
            ))}
          </div>
          <div className="text-center text-xs mb-2" style={{ color: '#C73E74' }}>
            {water} / 8 glasses 💧
          </div>
          ...
          >
            <div className="font-medium mb-1 flex items-center gap-1">
              <Sparkles size={11} /> Hydration tip
            </div>
            <p style={{ color: '#555' }}>
              {water < 4
                ? 'You need to drink more water! Try setting hourly reminders 💧'
                : water < 7
                ? 'You are doing great! Just a few more glasses to go 🌸'
        ...
        <Card className="lg:col-span-2">
          <SectionHeader
            ...
            icon={<Heart size={13} />}
            action={
              <Button variant="ghost" onClick={() => setShowAddWellness((v) => !v)}>
                <Plus size={12} />
              </Button>
            }
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5 mb-3">
            {wellness.map((w) => (
              <div
                ...
                onClick={() =>
                  setWellness((ws) =>
                    ws.map((x) => (x.id === w.id ? { ...x, done: !x.done } : x))
                  )
                }
              ...
              >
                <div
                  className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 border transition-all text-[10px]"
                  style={{
                    background: w.done ? '#EA4C89' : 'transparent',
                    borderColor: w.done ? '#EA4C89' : '#F5C8E2',
                    color: '#fff',
                  }}
                >
                  {w.done && '✓'}
                ...
                <span
                  className={`text-xs flex-1 ${w.done ? 'line-through text-gray-300' : ''}`}
                  style={{ color: w.done ? undefined : '#555' }}
                >
                  {w.text}
                </span>
              </div>
          ...
          <div>
            <div className="flex justify-between text-[10px] mb-1" style={{ color: '#888' }}>
              ...
              <span style={{ color: '#EA4C89' }}>
                {Math.round((wellnessDone / wellness.length) * 100)}%
              </span>
            </div>
        ...
        <Card>
          <SectionHeader
            ...
            icon={<Moon size={13} />}
            action={
              <Button variant="ghost" onClick={() => setShowAddSleep((v) => !v)}>
                <Plus size={12} />
              </Button>
            }
          />
              ...
              >
                ...
                >
                  <Input
                    label="Date"
                    value={newSleep.date}
                    onChange={(v) => setNewSleep((x) => ({ ...x, date: v }))}
                    type="date"
                  />
                  <Input
                    label="Hours slept"
                    value={newSleep.hours}
                    onChange={(v) => setNewSleep((x) => ({ ...x, hours: v }))}
                    type="number"
                    placeholder="7.5"
                  />
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((q) => (
                        <button
                          key={q}
                          onClick={() => setNewSleep((x) => ({ ...x, quality: String(q) }))}
                          className="w-8 h-8 rounded-full text-sm border transition-all"
                          style={{
                            borderColor:
                              newSleep.quality === String(q) ? '#EA4C89' : '#F5C8E2',
                            background:
                              newSleep.quality === String(q) ? '#FBEAF0' : 'transparent',
                          }}
                        >
                          {QUALITY_EMOJI[q]}
                        </button>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="ghost" onClick={() => setShowAddSleep(false)}>
                      Cancel
                    </Button>
                  </div>
                </div>
          ...
          >
            <div className="text-center flex-1">
              <div className="text-lg font-semibold" style={{ color: '#EA4C89' }}>
                {avgSleep.toFixed(1)}h
              </div>
              <div className="text-[10px]" style={{ color: '#888' }}>Avg sleep</div>
            </div>
            <div className="text-center flex-1">
              <div className="text-lg font-semibold" style={{ color: '#D8C1E8' }}>
              ...
              >
                ...
                >
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ width: `${(s.hours / 10) * 100}%` }}
                    transition={{ duration: 0.6 }}
                    style={{
                      background:
                        s.hours >= 7 ? '#D8C1E8' : s.hours >= 6 ? '#F5C8E2' : '#FBEAF0',
                    }}
                  />
                </div>
                <div
                ...
                <div className="text-sm flex-shrink-0">{QUALITY_EMOJI[s.quality]}</div>
              </div>
            ))}
          </div>
        </Card>
